...
double dynBetaCalculation(sVehicleParameters pVehicleParameters_s, sMeasuredValues pPrevMeasuredValues_s, sModelStates pPrevModelStates_s, double pTs_d) {
	...
	if (cos(pPrevModelStates_s.beta_d) != 0) {
		lVehicleSpeed_d = pPrevMeasuredValues_s.vehicleSpeed_d / cos(pPrevModelStates_s.beta_d); // vx / cos(beta)
	}
	//lVehicleSpeed = pPrevMeasuredValues_s.vehicleSpeed_d;

	if ((lVehicleSpeed_d != 0) && (((pVehicleParameters_s.m_d * (lVehicleSpeed_d * lVehicleSpeed_d)) - 1) != 0)) {
		lReturnValue_d =
			(pPrevMeasuredValues_s.steeringAngle_d *
				pTs_d * pVehicleParameters_s.c1_d / (pVehicleParameters_s.m_d * lVehicleSpeed_d))
			+
			((1 - pTs_d * ((pVehicleParameters_s.c1_d + pVehicleParameters_s.c2_d) / (pVehicleParameters_s.m_d * lVehicleSpeed_d))) *
				pPrevModelStates_s.beta_d)

			+
			((((pVehicleParameters_s.c2_d * pVehicleParameters_s.l2_d - pVehicleParameters_s.c1_d * pVehicleParameters_s.l1_d) / (pVehicleParameters_s.m_d * (lVehicleSpeed_d * lVehicleSpeed_d)) - 1) * pTs_d) *
