...
def generate_launch_description():
    return LaunchDescription([
        launch_ros.actions.Node(
            # namespace= "kalman_pos",
            package='kalman_pos', executable='kalman_pos_node', output='screen',
            parameters=[
                ...
                {"loop_rate_hz" : 60},
                {"estimation_method" : 6}, # 6: Kinematic + dynamic model with EKF and without GNSS position; initial GNSS based orientation estimation disabled (USE THIS AS DEFAULT FOR ESTIMATION WITHOUT GNSS)
                {"gnss_source" : "none"},
